...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://aircoinstallatiesittard.nl'
  ...
  // Generate city URLs with dynamic priorities based on population and focus on Sittard
  const cityUrls = cities.map((city) => ({
    url: `${siteUrl}/steden/${city.slug}`,
    lastModified,
    changeFrequency: 'weekly' as const,
    priority: city.city.toLowerCase() === 'sittard' ? 1.0 :
  ...
  // Generate service-city combination URLs with dynamic priorities and focus on Sittard
  const serviceCityUrls = diensten.flatMap((dienst) =>
    cities.map((city) => ({
      url: `${siteUrl}/diensten/${dienst.slug}/${city.slug}`,
      lastModified,
      changeFrequency: 'weekly' as const,
      priority: city.city.toLowerCase() === 'sittard' ? 0.9 :
